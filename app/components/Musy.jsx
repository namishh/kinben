...
const Musy = () => {
  ...
  useEffect(() => {
    id3.fromUrl(`http://localhost:3000/${track}.mp3`).then((tags) => {
      console.log(tags)
      setMetdata(tags)
    });
  }, [])
  ...
  return <div className="musy absolute bottom-[1rem] right-[1rem]">
    <audio preload="auto" onTimeUpdate={songTimeHandler} onLoadedMetadata={songTimeHandler} ref={audioRef} src={`http://localhost:3000/${track}.mp3`}></audio>
    {open && <div className="musicplayer absolute bottom-[3.9rem] flex justify-between flex-col rounded-lg -right-[1rem] z-[123123] md:right-0 h-[20rem] w-screen p-4 md:w-[28rem]" style={{ background: 'linear-gradient(to bottom right, #000000dd, #000000aa), url("./musicpic.png")', backgroundPosition: "top" }}>
      ...
      <div className="flex flex-col">
        ...
        <div className="flex gap-5 items-center justify-center">
          <SkipBack className="cursor-pointer" size={22} weight="fill" />
          ...
          <SkipForward className="cursor-pointer" size={22} weight="fill" />
        </div>
      </div>
    </div>
    }
